...
def silenceRemoval(x, fs, st_win, st_step, smoothWindow=0.5, weight=0.5, plot=False):
    ...
    st_feats, _ = sF.feature_extraction(x, fs, st_win * fs, st_step * fs) # st_feats (68个特征,966)
    ...
    [faets_s_norm, means_s, stds_s] = aT.normalizeFeatures(faets_s) # 标准化：减均值除方差
    ...
    T = (np.mean((1 - weight) * prog_on_set_sort[0:Nt]) +   # 排序后取 前96帧
